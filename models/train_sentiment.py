...
Trains multiple classifiers (Logistic Regression, Naive Bayes, SVM, Decision Tree, Random Forest)
on product review data, compares F1 scores, and saves the best model.
"""
...
def main():
    ...
    joblib.dump(best_model, os.path.join(models_dir, 'sentiment_model.pkl'))
    joblib.dump(tfidf, os.path.join(models_dir, 'tfidf_sentiment.pkl'))
    
    print(f"  → Saved sentiment_model.pkl ({best_model_name})")
    print(f"  → Saved tfidf_sentiment.pkl")
    print(f"\n{'=' * 60}")
